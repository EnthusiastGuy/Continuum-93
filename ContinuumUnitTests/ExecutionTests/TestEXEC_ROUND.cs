...
namespace ExecutionTests
{
    ...
    public class TestEXEC_ROUND
    {
        [Fact]
        public void TestEXEC_ROUND_fr()
        {
            for (byte i = 0; i < 16; i++)
            {
                Assembler cp = new();
                ...
                Assert.Equal(MathF.Round(value), computer.CPU.FREGS.GetRegister(i));
                TUtils.IncrementCountedTests("exec");
            }
        }

        [Fact]
        public void TestEXEC_ROUND_fr_fr()
        {
            Assembler cp = new();
            using Computer computer = new();
