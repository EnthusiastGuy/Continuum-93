...
namespace ExecutionTests
{
    ...
    public class TestEXEC_CEIL
    {
        [Fact]
        public void TestEXEC_CEIL_fr()
        {
            for (byte i = 0; i < 16; i++)
            {
                Assembler cp = new();
                ...
                Assert.Equal(MathF.Ceiling(value), computer.CPU.FREGS.GetRegister(i));
                TUtils.IncrementCountedTests("exec");
            }
        }

        [Fact]
        public void TestEXEC_CEIL_fr_fr()
        {
            Assembler cp = new();
            using Computer computer = new();
            ...
            Assert.Equal(2.0f, computer.CPU.FREGS.GetRegister(1));
            Assert.Equal(1.2f, computer.CPU.FREGS.GetRegister(0));
            Assert.Equal(2.0f, computer.CPU.FREGS.GetRegister(3));
