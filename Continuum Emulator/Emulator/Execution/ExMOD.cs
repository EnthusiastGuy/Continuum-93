...
{
    /// <summary>
    /// MOD execution unit aligned with the shared Instructions sub-opcodes
    /// (the same addressing matrix used by LD/ADD/SUB/DIV).
    ///
    /// - Integer modulo is unsigned and stores the remainder back into the destination.
    ...
    public static class ExMOD
    {
        ...
        private static void RegisterRegisterDestinations(Action<Computer>[] table)
        {
            ...
            table[Instructions._r_fr] = cpu => IntegerModByFloat(cpu, Width.Byte);
            ...
            table[Instructions._rr_fr] = cpu => IntegerModByFloat(cpu, Width.Word);
            ...
            table[Instructions._rrr_fr] = cpu => IntegerModByFloat(cpu, Width.TriByte);
            ...
            table[Instructions._rrrr_fr] = cpu => IntegerModByFloat(cpu, Width.DWord);
        }
        ...
        private static void RegisterMemoryGroups(Action<Computer>[] table)
        {
            ...
            foreach (var group in groups)
            {
                ...
                // Memory sourced block modulo with repeat
        ...
        private static Action<Computer> RegHandler(Width width, Func<MemoryController, Registers, (byte dest, uint value)> resolver)
        {
            return cpu =>
            {
                ...
                ModRegister(cpu, width, dest, value);
            };
        }
        ...
        private static Action<Computer> MemHandler(Width width, Func<MemoryController, Registers, (uint address, uint value)> resolver)
        {
            return cpu =>
            {
                ...
                ModMemory(cpu, width, address, value);
            };
        }
        ...
        private static Action<Computer> BlockHandler(Func<MemoryController, Registers, (uint address, uint valueOrAddress, byte count, uint repeat, bool sourceIsAddress)> resolver)
        {
            return cpu =>
            {
                ...
                ModBlock(cpu, address, valueOrAddress, count, repeat, sourceIsAddress);
            };
        }
        ...
        private static void ModRegister(Computer cpu, Width width, byte dest, uint divisor)
        {
            var regs = cpu.CPU.REGS;
        ...
        private static void ModMemory(Computer cpu, Width width, uint address, uint divisor)
        {
            var mem = cpu.MEMC;
            var flags = cpu.CPU.FLAGS;
        ...
        private static void ModBlock(Computer cpu, uint address, uint valueOrAddress, byte count, uint repeat, bool sourceIsAddress)
        {
            var mem = cpu.MEMC;
            var flags = cpu.CPU.FLAGS;
            ...
            for (uint r = 0; r < repeat; r++)
            {
                ...
                byte[] outBytes = ToBigEndianUnsignedFixed(remainder, count);
                for (int i = 0; i < count; i++)
        ...
        private static void IntegerModByFloat(Computer cpu, Width width)
        {
            var mem = cpu.MEMC;
            var regs = cpu.CPU.REGS;
            var fregs = cpu.CPU.FREGS;
            ...
            switch (width)
            {
                case Width.Byte:
                {
                    ...
                    float rem = v % divisor;
                    regs.Set8BitRegister(dest, (byte)Math.Round(rem));
                    break;
                }
                case Width.Word:
                {
                    ...
                    float rem = v % divisor;
                    regs.Set16BitRegister(dest, (ushort)Math.Round(rem));
                    break;
                }
                case Width.TriByte:
                {
                    ...
                    float rem = v % divisor;
                    uint q = (uint)Math.Abs(Math.Round(rem));
