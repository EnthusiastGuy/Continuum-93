...
{
    /// <summary>
    /// RR execution unit (Rotate Right) built from ExRL.
    ///
    /// "Roll" semantics:
    /// - Bits that would be shifted out from the LSB side are wrapped around into the MSB side.
    ...
    ///
    /// Carry flag:
    /// - For scalar rotates: Carry is set to the last bit rotated out of the LSB side.
    ///   (i.e., the bit that wrapped into the MSB on the final step)
    /// - For block rotates: Carry is set to the last bit rotated out of the LSB side.
    /// </summary>
    public static class ExRR
    {
        ...
        private static void RegisterRegisterDestinations(Action<Computer>[] table)
        {
            ...
            table[Instructions._r_n] = cpu =>
            {
                ...
                RotateRegisterRight(cpu, Width.Byte, dest, count);
            };
            ...
            table[Instructions._r_r] = cpu =>
            {
                ...
                RotateRegisterRight(cpu, Width.Byte, dest, regs.Get8BitRegister(src));
            };
            ...
            table[Instructions._rr_nn] = cpu =>
            {
                ...
                RotateRegisterRight(cpu, Width.Word, dest, (byte)imm);
            };
            ...
            table[Instructions._rr_r] = cpu =>
            {
                ...
                byte dest = ReadRegIndex(mem);
                byte src = ReadRegIndex(mem);
                RotateRegisterRight(cpu, Width.Word, dest, regs.Get8BitRegister(src));
            };
            ...
            table[Instructions._rr_rr] = cpu =>
            {
                ...
                byte dest = ReadRegIndex(mem);
                byte src = ReadRegIndex(mem);
                RotateRegisterRight(cpu, Width.Word, dest, (byte)regs.Get16BitRegister(src));
            };
            ...
            table[Instructions._rr_fr] = cpu =>
            {
                ...
                byte fIdx = ReadFloatRegIndex(mem);
                byte count = FloatToCountByte(fregs.GetRegister(fIdx));
                RotateRegisterRight(cpu, Width.Word, dest, count);
            };
            ...
            table[Instructions._rrr_nnn] = cpu =>
            {
                ...
                RotateRegisterRight(cpu, Width.TriByte, dest, (byte)imm24);
            };
            ...
            table[Instructions._rrr_r] = cpu =>
            {
                ...
                byte dest = ReadRegIndex(mem);
                byte src = ReadRegIndex(mem);
                RotateRegisterRight(cpu, Width.TriByte, dest, regs.Get8BitRegister(src));
            };
            ...
            table[Instructions._rrr_rr] = cpu =>
            {
                ...
                byte dest = ReadRegIndex(mem);
                byte src = ReadRegIndex(mem);
                RotateRegisterRight(cpu, Width.TriByte, dest, (byte)regs.Get16BitRegister(src));
            };
            ...
            table[Instructions._rrr_rrr] = cpu =>
            {
                ...
                byte dest = ReadRegIndex(mem);
                byte src = ReadRegIndex(mem);
                RotateRegisterRight(cpu, Width.TriByte, dest, (byte)regs.Get24BitRegister(src));
            };
            ...
            table[Instructions._rrr_fr] = cpu =>
            {
                ...
                byte fIdx = ReadFloatRegIndex(mem);
                byte count = FloatToCountByte(fregs.GetRegister(fIdx));
                RotateRegisterRight(cpu, Width.TriByte, dest, count);
            };
            ...
            table[Instructions._rrrr_nnnn] = cpu =>
            {
                ...
                RotateRegisterRight(cpu, Width.DWord, dest, (byte)imm32);
            };
            ...
            table[Instructions._rrrr_r] = cpu =>
            {
                ...
                byte dest = ReadRegIndex(mem);
                byte src = ReadRegIndex(mem);
                RotateRegisterRight(cpu, Width.DWord, dest, regs.Get8BitRegister(src));
            };
            ...
            table[Instructions._rrrr_rr] = cpu =>
            {
                ...
                byte dest = ReadRegIndex(mem);
                byte src = ReadRegIndex(mem);
                RotateRegisterRight(cpu, Width.DWord, dest, (byte)regs.Get16BitRegister(src));
            };
            ...
            table[Instructions._rrrr_rrr] = cpu =>
            {
                ...
                byte dest = ReadRegIndex(mem);
                byte src = ReadRegIndex(mem);
                RotateRegisterRight(cpu, Width.DWord, dest, (byte)regs.Get24BitRegister(src));
            };
            ...
            table[Instructions._rrrr_rrrr] = cpu =>
            {
                ...
                byte dest = ReadRegIndex(mem);
                byte src = ReadRegIndex(mem);
                RotateRegisterRight(cpu, Width.DWord, dest, (byte)regs.Get32BitRegister(src));
            };
            ...
            table[Instructions._rrrr_fr] = cpu =>
            {
                ...
                byte fIdx = ReadFloatRegIndex(mem);
                byte count = FloatToCountByte(fregs.GetRegister(fIdx));
                RotateRegisterRight(cpu, Width.DWord, dest, count);
            };
        }
        ...
        private static void RotateRegFromMem8(Computer cpu, Width destWidth, AddressResolver resolver)
        {
            ...
            RotateRegisterRight(cpu, destWidth, dest, count);
        }
        ...
        private static void RegisterMemoryGroups(Action<Computer>[] table)
        {
            ...
            foreach (var group in groups)
            {
                ...
                // Memory-sourced block rotates with repeat in rrr (same shape as ExRL)
                ...
                table[group.FloatOpcode] = cpu =>
                {
                    ...
                    bits = RotateRightU32(bits, count, out bool carry);
        ...
        {
            // RR fr, fr  (bitwise on IEEE754 bits)
            table[Instructions._fr_fr] = cpu =>
            {
                ...
                bool carry;
                bits = RotateRightU32(bits, count, out carry);
                cpu.CPU.FLAGS.SetCarry(carry);

                fregs.SetRegister(dest, FloatPointUtils.UintToFloat(bits));
            };

            // RR fr, nnnn  (count from 32-bit immediate; low byte used)
            table[Instructions._fr_nnnn] = cpu =>
            {
                ...
                bool carry;
                bits = RotateRightU32(bits, (byte)(rotCount & 0xFF), out carry);
                cpu.CPU.FLAGS.SetCarry(carry);

                fregs.SetRegister(dest, FloatPointUtils.UintToFloat(bits));
            };

            // RR fr, r / rr / rrr / rrrr  (count from integer regs; low byte used)
            table[Instructions._fr_r] = cpu =>
            {
                ...
                bool carry;
                bits = RotateRightU32(bits, regs.Get8BitRegister(src), out carry);
                cpu.CPU.FLAGS.SetCarry(carry);
            ...
            table[Instructions._fr_rr] = cpu =>
            {
                ...
                bool carry;
                bits = RotateRightU32(bits, (byte)regs.Get16BitRegister(src), out carry);
                cpu.CPU.FLAGS.SetCarry(carry);
            ...
            table[Instructions._fr_rrr] = cpu =>
            {
                ...
                bool carry;
                bits = RotateRightU32(bits, (byte)regs.Get24BitRegister(src), out carry);
                cpu.CPU.FLAGS.SetCarry(carry);
            ...
            table[Instructions._fr_rrrr] = cpu =>
            {
                ...
                bool carry;
                bits = RotateRightU32(bits, (byte)regs.Get32BitRegister(src), out carry);
                cpu.CPU.FLAGS.SetCarry(carry);

                fregs.SetRegister(dest, FloatPointUtils.UintToFloat(bits));
            };

            // RR fr, (mem8) via the shared address resolvers (count read as 8-bit from memory)
                ...
                case Width.Byte:
                    {
                        ...
                        byte r = RotateRightU8(v, count, out carry);
                        cpu.CPU.FLAGS.SetCarry(carry);
                ...
                case Width.Word:
                    {
                        ...
                        ushort r = RotateRightU16(v, count, out carry);
                        cpu.CPU.FLAGS.SetCarry(carry);
                ...
                case Width.TriByte:
                    {
                        ...
                        uint r = RotateRightU24(v, count, out carry);
                        cpu.CPU.FLAGS.SetCarry(carry);
                ...
                case Width.DWord:
                    {
                        ...
                        uint r = RotateRightU32(v, count, out carry);
                        cpu.CPU.FLAGS.SetCarry(carry);
        ...
        private static void RotateBlock(Computer cpu, uint address, uint valueOrAddress, byte countBytes, uint repeat, bool sourceIsAddress)
        {
            ...
            {
                // REQUIRED SEMANTICS: rotation count is a SINGLE BYTE at valueOrAddress
            ...
            // Carry = last bit rotated out of the LSB side.
            // In MSB-indexing, LSB side bit at position (rot-1) from LSB corresponds to MSB-index (bitWidth - rot).
            bool carry = GetBitBigEndian(mem, address, countBytes, (int)(bitWidth - rot));
            cpu.CPU.FLAGS.SetCarry(carry);

            RotateRightBigEndianInPlace(mem, address, countBytes, (int)rot);
        }

        private static void RotateRegisterRight(Computer cpu, Width width, byte regIndex, byte rawCount)
        {
            var regs = cpu.CPU.REGS;
        ...
        // -------------------------
        // Scalar rotate helpers + carry (RR)
        // -------------------------
        private static byte RotateRightU8(byte v, int count, out bool carry)
        {
            count &= 7;
            if (count == 0) { carry = false; return v; }
            carry = ((v >> (count - 1)) & 0x1) != 0;
            return (byte)((v >> count) | (v << (8 - count)));
        }

        private static ushort RotateRightU16(ushort v, int count, out bool carry)
        {
            count &= 15;
            if (count == 0) { carry = false; return v; }
            carry = ((v >> (count - 1)) & 0x1) != 0;
        ...
        private static uint RotateRightU24(uint v, int count, out bool carry)
        {
            count %= 24;
            v &= 0xFFFFFFu;
            if (count == 0) { carry = false; return v; }
            carry = ((v >> (count - 1)) & 0x1u) != 0;
            uint r = (v >> count) | (v << (24 - count));
            return r & 0xFFFFFFu;
        }

        private static uint RotateRightU32(uint v, byte rawCount, out bool carry)
        {
        ...
        // -------------------------
        // Big-endian block rotate helpers (RR)
        // -------------------------
        ...
        private static void RotateRightBigEndianInPlace(MemoryController mem, uint baseAddr, byte countBytes, int rotBits)
        {
            ...
            // Right rotate:
            // dest[p] = src[(p - rotBits + bitWidth) % bitWidth]   (p is MSB-indexed)
