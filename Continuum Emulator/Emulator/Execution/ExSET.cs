...
{
    /// <summary>
    /// SET execution unit updated to use the shared Instructions sub-opcodes
    ...
    ///
    /// Semantics implemented here (bit-set):
    /// - Register destinations: set (to 1) the bit indexed by a bit-position resolved from
    ///   immediate / register / memory / float-reg (per sub-op), in the destination register.
    /// - Memory destinations: set (to 1) the bit indexed by the resolved bit-position in the
    ...
    public static class ExSET
    {
        ...
        private static void RegisterRegisterDestinations(Action<Computer>[] table)
        {
            ...
            table[Instructions._r_n] = cpu =>
            {
                ...
                SetRegisterBit(cpu, Width.Byte, dest, bitIndex);
            };
            ...
            table[Instructions._r_r] = cpu =>
            {
                ...
                SetRegisterBit(cpu, Width.Byte, dest, regs.Get8BitRegister(src));
            };

            table[Instructions._r_InnnI] = cpu => SetRegBitFromMem8(cpu, Width.Byte, AddressAbs);
            table[Instructions._r_Innn_nnnI] = cpu => SetRegBitFromMem8(cpu, Width.Byte, AddressAbsOffsetImm);
            table[Instructions._r_Innn_rI] = cpu => SetRegBitFromMem8(cpu, Width.Byte, AddressAbsOffsetReg8);
            table[Instructions._r_Innn_rrI] = cpu => SetRegBitFromMem8(cpu, Width.Byte, AddressAbsOffsetReg16);
            table[Instructions._r_Innn_rrrI] = cpu => SetRegBitFromMem8(cpu, Width.Byte, AddressAbsOffsetReg24);

            table[Instructions._r_IrrrI] = cpu => SetRegBitFromMem8(cpu, Width.Byte, AddressPtr);
            table[Instructions._r_Irrr_nnnI] = cpu => SetRegBitFromMem8(cpu, Width.Byte, AddressPtrOffsetImm);
            table[Instructions._r_Irrr_rI] = cpu => SetRegBitFromMem8(cpu, Width.Byte, AddressPtrOffsetReg8);
            table[Instructions._r_Irrr_rrI] = cpu => SetRegBitFromMem8(cpu, Width.Byte, AddressPtrOffsetReg16);
            table[Instructions._r_Irrr_rrrI] = cpu => SetRegBitFromMem8(cpu, Width.Byte, AddressPtrOffsetReg24);
            ...
            table[Instructions._rr_nn] = cpu =>
            {
                ...
                SetRegisterBit(cpu, Width.Word, dest, (byte)imm);
            };
            ...
            table[Instructions._rr_r] = cpu =>
            {
                ...
                byte dest = ReadRegIndex(mem);
                byte src = ReadRegIndex(mem);
                SetRegisterBit(cpu, Width.Word, dest, regs.Get8BitRegister(src));
            };
            ...
            table[Instructions._rr_rr] = cpu =>
            {
                ...
                byte dest = ReadRegIndex(mem);
                byte src = ReadRegIndex(mem);
                SetRegisterBit(cpu, Width.Word, dest, (byte)regs.Get16BitRegister(src));
            };

            table[Instructions._rr_InnnI] = cpu => SetRegBitFromMem8(cpu, Width.Word, AddressAbs);
            table[Instructions._rr_Innn_nnnI] = cpu => SetRegBitFromMem8(cpu, Width.Word, AddressAbsOffsetImm);
            table[Instructions._rr_Innn_rI] = cpu => SetRegBitFromMem8(cpu, Width.Word, AddressAbsOffsetReg8);
            table[Instructions._rr_Innn_rrI] = cpu => SetRegBitFromMem8(cpu, Width.Word, AddressAbsOffsetReg16);
            table[Instructions._rr_Innn_rrrI] = cpu => SetRegBitFromMem8(cpu, Width.Word, AddressAbsOffsetReg24);

            table[Instructions._rr_IrrrI] = cpu => SetRegBitFromMem8(cpu, Width.Word, AddressPtr);
            table[Instructions._rr_Irrr_nnnI] = cpu => SetRegBitFromMem8(cpu, Width.Word, AddressPtrOffsetImm);
            table[Instructions._rr_Irrr_rI] = cpu => SetRegBitFromMem8(cpu, Width.Word, AddressPtrOffsetReg8);
            table[Instructions._rr_Irrr_rrI] = cpu => SetRegBitFromMem8(cpu, Width.Word, AddressPtrOffsetReg16);
            table[Instructions._rr_Irrr_rrrI] = cpu => SetRegBitFromMem8(cpu, Width.Word, AddressPtrOffsetReg24);
            ...
            table[Instructions._rr_fr] = cpu =>
            {
                ...
                byte fIdx = ReadFloatRegIndex(mem);
                byte bitIndex = FloatToIndexByte(fregs.GetRegister(fIdx));
                SetRegisterBit(cpu, Width.Word, dest, bitIndex);
            };
            ...
            table[Instructions._rrr_nnn] = cpu =>
            {
                ...
                SetRegisterBit(cpu, Width.TriByte, dest, (byte)imm24);
            };
            ...
            table[Instructions._rrr_r] = cpu =>
            {
                ...
                byte dest = ReadRegIndex(mem);
                byte src = ReadRegIndex(mem);
                SetRegisterBit(cpu, Width.TriByte, dest, regs.Get8BitRegister(src));
            };
            ...
            table[Instructions._rrr_rr] = cpu =>
            {
                ...
                byte dest = ReadRegIndex(mem);
                byte src = ReadRegIndex(mem);
                SetRegisterBit(cpu, Width.TriByte, dest, (byte)regs.Get16BitRegister(src));
            };
            ...
            table[Instructions._rrr_rrr] = cpu =>
            {
                ...
                byte dest = ReadRegIndex(mem);
                byte src = ReadRegIndex(mem);
                SetRegisterBit(cpu, Width.TriByte, dest, (byte)regs.Get24BitRegister(src));
            };

            table[Instructions._rrr_InnnI] = cpu => SetRegBitFromMem8(cpu, Width.TriByte, AddressAbs);
            table[Instructions._rrr_Innn_nnnI] = cpu => SetRegBitFromMem8(cpu, Width.TriByte, AddressAbsOffsetImm);
            table[Instructions._rrr_Innn_rI] = cpu => SetRegBitFromMem8(cpu, Width.TriByte, AddressAbsOffsetReg8);
            table[Instructions._rrr_Innn_rrI] = cpu => SetRegBitFromMem8(cpu, Width.TriByte, AddressAbsOffsetReg16);
            table[Instructions._rrr_Innn_rrrI] = cpu => SetRegBitFromMem8(cpu, Width.TriByte, AddressAbsOffsetReg24);

            table[Instructions._rrr_IrrrI] = cpu => SetRegBitFromMem8(cpu, Width.TriByte, AddressPtr);
            table[Instructions._rrr_Irrr_nnnI] = cpu => SetRegBitFromMem8(cpu, Width.TriByte, AddressPtrOffsetImm);
            table[Instructions._rrr_Irrr_rI] = cpu => SetRegBitFromMem8(cpu, Width.TriByte, AddressPtrOffsetReg8);
            table[Instructions._rrr_Irrr_rrI] = cpu => SetRegBitFromMem8(cpu, Width.TriByte, AddressPtrOffsetReg16);
            table[Instructions._rrr_Irrr_rrrI] = cpu => SetRegBitFromMem8(cpu, Width.TriByte, AddressPtrOffsetReg24);
            ...
            table[Instructions._rrr_fr] = cpu =>
            {
                ...
                byte fIdx = ReadFloatRegIndex(mem);
                byte bitIndex = FloatToIndexByte(fregs.GetRegister(fIdx));
                SetRegisterBit(cpu, Width.TriByte, dest, bitIndex);
            };
            ...
            table[Instructions._rrrr_nnnn] = cpu =>
            {
                ...
                SetRegisterBit(cpu, Width.DWord, dest, (byte)imm32);
            };
            ...
            table[Instructions._rrrr_r] = cpu =>
            {
                ...
                byte dest = ReadRegIndex(mem);
                byte src = ReadRegIndex(mem);
                SetRegisterBit(cpu, Width.DWord, dest, regs.Get8BitRegister(src));
            };
            ...
            table[Instructions._rrrr_rr] = cpu =>
            {
                ...
                byte dest = ReadRegIndex(mem);
                byte src = ReadRegIndex(mem);
                SetRegisterBit(cpu, Width.DWord, dest, (byte)regs.Get16BitRegister(src));
            };
            ...
            table[Instructions._rrrr_rrr] = cpu =>
            {
                ...
                byte dest = ReadRegIndex(mem);
                byte src = ReadRegIndex(mem);
                SetRegisterBit(cpu, Width.DWord, dest, (byte)regs.Get24BitRegister(src));
            };
            ...
            table[Instructions._rrrr_rrrr] = cpu =>
            {
                ...
                byte dest = ReadRegIndex(mem);
                byte src = ReadRegIndex(mem);
                SetRegisterBit(cpu, Width.DWord, dest, (byte)regs.Get32BitRegister(src));
            };

            table[Instructions._rrrr_InnnI] = cpu => SetRegBitFromMem8(cpu, Width.DWord, AddressAbs);
            table[Instructions._rrrr_Innn_nnnI] = cpu => SetRegBitFromMem8(cpu, Width.DWord, AddressAbsOffsetImm);
            table[Instructions._rrrr_Innn_rI] = cpu => SetRegBitFromMem8(cpu, Width.DWord, AddressAbsOffsetReg8);
            table[Instructions._rrrr_Innn_rrI] = cpu => SetRegBitFromMem8(cpu, Width.DWord, AddressAbsOffsetReg16);
            table[Instructions._rrrr_Innn_rrrI] = cpu => SetRegBitFromMem8(cpu, Width.DWord, AddressAbsOffsetReg24);

            table[Instructions._rrrr_IrrrI] = cpu => SetRegBitFromMem8(cpu, Width.DWord, AddressPtr);
            table[Instructions._rrrr_Irrr_nnnI] = cpu => SetRegBitFromMem8(cpu, Width.DWord, AddressPtrOffsetImm);
            table[Instructions._rrrr_Irrr_rI] = cpu => SetRegBitFromMem8(cpu, Width.DWord, AddressPtrOffsetReg8);
            table[Instructions._rrrr_Irrr_rrI] = cpu => SetRegBitFromMem8(cpu, Width.DWord, AddressPtrOffsetReg16);
            table[Instructions._rrrr_Irrr_rrrI] = cpu => SetRegBitFromMem8(cpu, Width.DWord, AddressPtrOffsetReg24);
            ...
            table[Instructions._rrrr_fr] = cpu =>
            {
                ...
                byte fIdx = ReadFloatRegIndex(mem);
                byte bitIndex = FloatToIndexByte(fregs.GetRegister(fIdx));
                SetRegisterBit(cpu, Width.DWord, dest, bitIndex);
            };
        }

        private static void SetRegBitFromMem8(Computer cpu, Width destWidth, AddressResolver resolver)
        {
            var mem = cpu.MEMC;
            var regs = cpu.CPU.REGS;
            byte dest = ReadRegIndex(mem);
            ...
            SetRegisterBit(cpu, destWidth, dest, bitIndex);
        }
        ...
        private static void RegisterMemoryGroups(Action<Computer>[] table)
        {
            ...
            foreach (var group in groups)
            {
                ...
                // Register->memory bit-set (bit index from register; low byte used)
                ...
                // Float->memory bit-set: treat memory float as bits, set bit by float-reg-derived index
        ...
        {
            // SET fr, fr  (bit index from src float; modifies dest float bits)
            ...
            // SET fr, nnnn  (bit index from 32-bit immediate; low byte used)
            table[Instructions._fr_nnnn] = cpu =>
            {
                ...
                uint bits = FloatPointUtils.FloatToUint(fregs.GetRegister(dest));
                bits = SetBitU32(bits, (byte)(bitIndex & 0xFF));
                fregs.SetRegister(dest, FloatPointUtils.UintToFloat(bits));
            };

            // SET fr, r / rr / rrr / rrrr  (bit index from integer regs; low byte used)
            table[Instructions._fr_r] = cpu =>
            {
                ...
                uint bits = FloatPointUtils.FloatToUint(fregs.GetRegister(dest));
                bits = SetBitU32(bits, regs.Get8BitRegister(src));
                fregs.SetRegister(dest, FloatPointUtils.UintToFloat(bits));
            };
            ...
            table[Instructions._fr_rr] = cpu =>
            {
                ...
                uint bits = FloatPointUtils.FloatToUint(fregs.GetRegister(dest));
                bits = SetBitU32(bits, (byte)regs.Get16BitRegister(src));
                fregs.SetRegister(dest, FloatPointUtils.UintToFloat(bits));
            };
            ...
            table[Instructions._fr_rrr] = cpu =>
            {
                ...
                uint bits = FloatPointUtils.FloatToUint(fregs.GetRegister(dest));
                bits = SetBitU32(bits, (byte)regs.Get24BitRegister(src));
                fregs.SetRegister(dest, FloatPointUtils.UintToFloat(bits));
            };
            ...
            table[Instructions._fr_rrrr] = cpu =>
            {
                ...
                uint bits = FloatPointUtils.FloatToUint(fregs.GetRegister(dest));
                bits = SetBitU32(bits, (byte)regs.Get32BitRegister(src));
                fregs.SetRegister(dest, FloatPointUtils.UintToFloat(bits));
            };

            // SET fr, (mem8) via shared address resolvers (bit index read as 8-bit from memory)
        ...
        private static Action<Computer> MemHandler(Width width, Func<MemoryController, Registers, (uint address, uint bitIndex)> resolver)
        {
            return cpu =>
            {
                ...
                SetMemoryBit(cpu, width, address, (byte)(bitIndex & 0xFF));
            };
        }
        ...
        private static Action<Computer> BlockHandler(Func<MemoryController, Registers, (uint address, uint valueOrAddress, byte count, uint repeat, bool sourceIsAddress)> resolver)
        {
            return cpu =>
            {
                ...
                SetBlockBit(cpu, address, valueOrAddress, count, repeat, sourceIsAddress);
            };
        }

        private static void SetMemoryBit(Computer cpu, Width width, uint address, byte rawBitIndex)
        {
            var mem = cpu.MEMC;
        ...
        private static void SetBlockBit(Computer cpu, uint address, uint valueOrAddress, byte count, uint repeat, bool sourceIsAddress)
        {
            var mem = cpu.MEMC;
            ...
            // With repeat we set a run of bits starting at bitIndex
            for (uint i = 0; i < repeat; i++)
            {
                uint idx = (bitIndex + i) % bitWidth;
                SetBitBigEndianInPlace(mem, address, count, (int)idx);
            }
        }

        private static void SetBitBigEndianInPlace(MemoryController mem, uint baseAddr, byte countBytes, int bitIndex)
        {
            ...
            mem.Set8bitToRAM(addr, (byte)(b | mask));
        }

        // -------------------------
        // Core register bit-set helper
        // -------------------------
        private static void SetRegisterBit(Computer cpu, Width width, byte regIndex, byte rawBitIndex)
        {
            var regs = cpu.CPU.REGS;

            // Keep existing register helper semantics if present.
            // The Set*BitBit methods are assumed to perform modulo and flag behavior as designed in REGs.
            switch (width)
            {
                case Width.Byte: regs.Set8BitBit(regIndex, rawBitIndex); break;
                case Width.Word: regs.Set16BitBit(regIndex, rawBitIndex); break;
                case Width.TriByte: regs.Set24BitBit(regIndex, rawBitIndex); break;
                case Width.DWord: regs.Set32BitBit(regIndex, rawBitIndex); break;
            }
        }
        ...
        private static uint SetBitU32(uint v, byte rawBitIndex)
        {
