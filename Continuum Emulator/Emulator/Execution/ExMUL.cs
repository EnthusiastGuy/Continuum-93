...
{
    /// <summary>
    /// MUL execution unit aligned with the shared Instructions sub-opcodes
    /// (the same addressing matrix used by LD/ADD/SUB/DIV).
    ///
    /// - Integer multiplication is unsigned and stores the product back into the destination.
    /// - Overflow sets the carry flag.
    /// - Float multiplication sets the sign flag based on result.
    /// </summary>
    public static class ExMUL
    {
        ...
        private static void RegisterRegisterDestinations(Action<Computer>[] table)
        {
            ...
            table[Instructions._r_fr] = cpu => IntegerMulByFloat(cpu, Width.Byte);
            ...
            table[Instructions._rr_fr] = cpu => IntegerMulByFloat(cpu, Width.Word);
            ...
            table[Instructions._rrr_fr] = cpu => IntegerMulByFloat(cpu, Width.TriByte);
            ...
            table[Instructions._rrrr_fr] = cpu => IntegerMulByFloat(cpu, Width.DWord);
        }
        ...
        private static void RegisterMemoryGroups(Action<Computer>[] table)
        {
            ...
            foreach (var group in groups)
            {
                ...
                // Memory sourced block multiplies with repeat
                ...
                table[group.FloatOpcode] = cpu =>
                {
                    ...
                    float multiplier = fregs.GetRegister(fIndex);

                    float current = mem.GetFloatFromRAM(address);
                    float result = current * multiplier;
        ...
        private static void RegisterFloatRegisterHandlers(Action<Computer>[] table)
        {
            ...
            table[Instructions._fr_fr] = cpu =>
            {
                ...
                float result = fregs.GetRegister(dest) * fregs.GetRegister(src);
                cpu.CPU.FLAGS.SetSignNegative(result < 0);
            ...
            table[Instructions._fr_nnnn] = cpu =>
            {
                ...
                float multiplier = FloatPointUtils.UintToFloat(bits);

                float result = fregs.GetRegister(dest) * multiplier;
                cpu.CPU.FLAGS.SetSignNegative(result < 0);
                fregs.SetRegister(dest, result);
            };
            ...
            table[Instructions._fr_r] = cpu =>
            {
                ...
                float multiplier = regs.Get8BitRegister(src);
                float result = fregs.GetRegister(dest) * multiplier;
                cpu.CPU.FLAGS.SetSignNegative(result < 0);
                fregs.SetRegister(dest, result);
            };
            ...
            table[Instructions._fr_rr] = cpu =>
            {
                ...
                byte dest = ReadFloatRegIndex(mem);
                byte src = ReadRegIndex(mem);
                float multiplier = regs.Get16BitRegister(src);
                float result = fregs.GetRegister(dest) * multiplier;
                cpu.CPU.FLAGS.SetSignNegative(result < 0);
                fregs.SetRegister(dest, result);
            };
            ...
            table[Instructions._fr_rrr] = cpu =>
            {
                ...
                byte dest = ReadFloatRegIndex(mem);
                byte src = ReadRegIndex(mem);
                float multiplier = regs.Get24BitRegister(src);
                float result = fregs.GetRegister(dest) * multiplier;
                cpu.CPU.FLAGS.SetSignNegative(result < 0);
                fregs.SetRegister(dest, result);
            };
            ...
            table[Instructions._fr_rrrr] = cpu =>
            {
                ...
                byte dest = ReadFloatRegIndex(mem);
                byte src = ReadRegIndex(mem);
                float multiplier = regs.Get32BitRegister(src);
                float result = fregs.GetRegister(dest) * multiplier;
                cpu.CPU.FLAGS.SetSignNegative(result < 0);
                fregs.SetRegister(dest, result);
            };
        ...
        private static Action<Computer> RegHandler(Width width, Func<MemoryController, Registers, (byte dest, uint value)> resolver)
        {
            return cpu =>
            {
                ...
                MultiplyRegister(cpu, width, dest, value);
            };
        }
        ...
        private static Action<Computer> MemHandler(Width width, Func<MemoryController, Registers, (uint address, uint value)> resolver)
        {
            return cpu =>
            {
                ...
                MultiplyMemory(cpu, width, address, value);
            };
        }
        ...
        private static Action<Computer> BlockHandler(Func<MemoryController, Registers, (uint address, uint valueOrAddress, byte count, uint repeat, bool sourceIsAddress)> resolver)
        {
            return cpu =>
            {
                ...
                MultiplyBlock(cpu, address, valueOrAddress, count, repeat, sourceIsAddress);
            };
        }
        ...
        private static Action<Computer> FloatRegMemHandler(AddressResolver resolver)
        {
            return cpu =>
            {
                ...
                float multiplier = mem.GetFloatFromRAM(address);
                float result = fregs.GetRegister(dest) * multiplier;
                cpu.CPU.FLAGS.SetSignNegative(result < 0);
                fregs.SetRegister(dest, result);
            };
        ...
        private static void MultiplyRegister(Computer cpu, Width width, byte dest, uint multiplier)
        {
            var regs = cpu.CPU.REGS;
            ...
            switch (width)
            {
                case Width.Byte:
                {
                    uint product = (uint)regs.Get8BitRegister(dest) * multiplier;
                    regs.Set8BitRegister(dest, (byte)product);
                    flags.SetCarry(product > byte.MaxValue);
                    break;
                }
                case Width.Word:
                {
                    uint product = (uint)regs.Get16BitRegister(dest) * multiplier;
                    regs.Set16BitRegister(dest, (ushort)product);
                    flags.SetCarry(product > ushort.MaxValue);
                    break;
                }
                case Width.TriByte:
                {
                    ulong product = (ulong)regs.Get24BitRegister(dest) * multiplier;
                    regs.Set24BitRegister(dest, (uint)product);
                    flags.SetCarry(product > 0xFFFFFF);
                    break;
                }
                case Width.DWord:
                {
                    ulong product = (ulong)regs.Get32BitRegister(dest) * multiplier;
                    regs.Set32BitRegister(dest, (uint)product);
                    flags.SetCarry(product > 0xFFFFFFFF);
                    break;
                }
            }
        }

        private static void MultiplyMemory(Computer cpu, Width width, uint address, uint multiplier)
        {
            var mem = cpu.MEMC;
            var flags = cpu.CPU.FLAGS;
            ...
            switch (width)
            {
                case Width.Byte:
                {
                    uint product = (uint)mem.Get8bitFromRAM(address) * multiplier;
                    mem.Set8bitToRAM(address, (byte)product);
                    flags.SetCarry(product > byte.MaxValue);
                    break;
                }
                case Width.Word:
                {
                    uint product = (uint)mem.Get16bitFromRAM(address) * multiplier;
                    mem.Set16bitToRAM(address, (ushort)product);
                    flags.SetCarry(product > ushort.MaxValue);
                    break;
                }
                case Width.TriByte:
                {
                    ulong product = (ulong)mem.Get24bitFromRAM(address) * multiplier;
                    mem.Set24bitToRAM(address, (uint)product);
                    flags.SetCarry(product > 0xFFFFFF);
                    break;
                }
                case Width.DWord:
                {
                    ulong product = (ulong)mem.Get32bitFromRAM(address) * multiplier;
                    mem.Set32bitToRAM(address, (uint)product);
                    flags.SetCarry(product > 0xFFFFFFFF);
                    break;
                }
            }
        ...
        private static void MultiplyBlock(Computer cpu, uint address, uint valueOrAddress, byte count, uint repeat, bool sourceIsAddress)
        {
            var mem = cpu.MEMC;
            var flags = cpu.CPU.FLAGS;
            ...
            {
                byte[] multiplicandBytes = ReadBytes(mem, address, count);
                BigInteger multiplicand = FromBigEndianUnsigned(multiplicandBytes);

                BigInteger multiplier = sourceIsAddress
                    ? FromBigEndianUnsigned(ReadBytes(mem, valueOrAddress, count))
                    : new BigInteger((ulong)valueOrAddress);

                BigInteger product = multiplicand * multiplier;
                byte[] outBytes = ToBigEndianUnsignedFixed(product, count);
                for (int i = 0; i < count; i++)
                    mem.Set8bitToRAM(address + (uint)i, outBytes[i]);

                // Set carry if result exceeds the byte width
                BigInteger maxValue = (BigInteger.One << (count * 8)) - 1;
                flags.SetCarry(product > maxValue);
            }
        }

        private static void IntegerMulByFloat(Computer cpu, Width width)
        {
            var mem = cpu.MEMC;
            var regs = cpu.CPU.REGS;
            var fregs = cpu.CPU.FREGS;
            ...
            float multiplier = fregs.GetRegister(fIdx);
