...
{
    /// <summary>
    /// SUB execution unit aligned with the shared Instructions sub-opcodes
    /// (the same addressing matrix used by LD/ADD). All handlers mirror
    /// ExADD but perform subtraction instead.
    /// </summary>
    public static class ExSUB
    {
        ...
        private static void RegisterMemoryGroups(Action<Computer>[] table)
        {
            ...
            foreach (var group in groups)
            {
                ...
                // Memory sourced block subtracts with repeat
                ...
                table[group.FloatOpcode] = cpu =>
                {
                    ...
                    float sub = fregs.GetRegister(fIndex);
                    mem.SetFloatToRam(address, fregs.SubFloatValues(current, sub));
                };
            }
        }
        ...
        private static void RegisterFloatRegisterHandlers(Action<Computer>[] table)
        {
            ...
            table[Instructions._fr_fr] = cpu =>
            {
                ...
                fregs.SetRegister(dest, fregs.SubFloatValues(fregs.GetRegister(dest), fregs.GetRegister(src)));
            };
            ...
            table[Instructions._fr_nnnn] = cpu =>
            {
                ...
                fregs.SetRegister(dest, fregs.SubFloatValues(fregs.GetRegister(dest), val));
            };
            ...
            table[Instructions._fr_r] = cpu =>
            {
                ...
                fregs.SetRegister(dest, fregs.SubFloatValues(fregs.GetRegister(dest), regs.Get8BitRegister(src)));
            };
            ...
            table[Instructions._fr_rr] = cpu =>
            {
                ...
                byte dest = ReadFloatRegIndex(mem);
                byte src = ReadRegIndex(mem);
                fregs.SetRegister(dest, fregs.SubFloatValues(fregs.GetRegister(dest), regs.Get16BitRegister(src)));
            };
            ...
            table[Instructions._fr_rrr] = cpu =>
            {
                ...
                byte dest = ReadFloatRegIndex(mem);
                byte src = ReadRegIndex(mem);
                fregs.SetRegister(dest, fregs.SubFloatValues(fregs.GetRegister(dest), regs.Get24BitRegister(src)));
            };
            ...
            table[Instructions._fr_rrrr] = cpu =>
            {
                ...
                byte dest = ReadFloatRegIndex(mem);
                byte src = ReadRegIndex(mem);
                fregs.SetRegister(dest, fregs.SubFloatValues(fregs.GetRegister(dest), regs.Get32BitRegister(src)));
            };
        ...
        private static Action<Computer> RegHandler(Width width, Func<MemoryController, Registers, (byte dest, uint value)> resolver)
        {
            return cpu =>
            {
                ...
                SubtractFromRegister(cpu, width, dest, value);
            };
        }
        ...
        private static Action<Computer> MemHandler(Width width, Func<MemoryController, Registers, (uint address, uint value)> resolver)
        {
            return cpu =>
            {
                ...
                SubtractFromMemory(cpu, width, address, value);
            };
        }
        ...
        private static Action<Computer> BlockHandler(Func<MemoryController, Registers, (uint address, uint valueOrAddress, byte count, uint repeat, bool sourceIsAddress)> resolver)
        {
            return cpu =>
            {
                ...
                SubtractBlock(cpu, address, valueOrAddress, count, repeat, sourceIsAddress);
            };
        }
        ...
        private static Action<Computer> FloatRegMemHandler(AddressResolver resolver)
        {
            return cpu =>
            {
                ...
                fregs.SetRegister(dest, fregs.SubFloatValues(fregs.GetRegister(dest), mem.GetFloatFromRAM(address)));
            };
        }

        private static void SubtractFromRegister(Computer cpu, Width width, byte dest, uint value)
        {
            var regs = cpu.CPU.REGS;
            switch (width)
            {
                case Width.Byte:
                    regs.SubtractFrom8BitRegister(dest, (byte)value);
                    break;
                case Width.Word:
                    regs.SubtractFrom16BitRegister(dest, (ushort)value);
                    break;
                case Width.TriByte:
                    regs.SubtractFrom24BitRegister(dest, value & 0xFFFFFF);
                    break;
                case Width.DWord:
                    regs.SubtractFrom32BitRegister(dest, value);
                    break;
            }
        }

        private static void SubtractFromMemory(Computer cpu, Width width, uint address, uint value)
        {
            var mem = cpu.MEMC;
            var regs = cpu.CPU.REGS;
            switch (width)
            {
                case Width.Byte:
                    mem.Set8bitToRAM(address, regs.Sub8BitValues(mem.Get8bitFromRAM(address), (byte)value));
                    break;
                case Width.Word:
                    mem.Set16bitToRAM(address, regs.Sub16BitValues(mem.Get16bitFromRAM(address), (ushort)value));
                    break;
                case Width.TriByte:
                    mem.Set24bitToRAM(address, regs.Sub24BitValues(mem.Get24bitFromRAM(address), value & 0xFFFFFF));
                    break;
                case Width.DWord:
                    mem.Set32bitToRAM(address, regs.Sub32BitValues(mem.Get32bitFromRAM(address), value));
                    break;
            }
        }

        private static void SubtractBlock(Computer cpu, uint address, uint valueOrAddress, byte count, uint repeat, bool sourceIsAddress)
        {
            var mem = cpu.MEMC;
            var regs = cpu.CPU.REGS;
            ...
            for (uint r = 0; r < repeat; r++)
            {
                ...
                byte borrow = 0;
                // propagate borrow from higher offsets down to lower offsets (most-significant first)
                for (int i = count - 1; i >= 0; i--)
                {
                    byte subByte = sourceIsAddress
                        ? mem.Get8bitFromRAM(sourceBase + (uint)i)
                        : (i >= 4 ? (byte)0 : (byte)(valueOrAddress >> (8 * (count - 1 - i))));
                    uint target = baseAddr + (uint)i;

                    byte dest = mem.Get8bitFromRAM(target);
                    byte borrowIn = borrow;
                    int diff = dest - subByte - borrowIn;
                    byte result = (byte)diff;
                    borrow = (byte)(diff < 0 ? 1 : 0);

                    flags.Update8BitSubFlags(dest, (byte)(subByte + borrowIn));
                    mem.Set8bitToRAM(target, result);
                }

                // if borrow remains after the last byte, reflect it in the carry flag
        ...
        private static void FloatToInteger(Computer cpu, Width width)
        {
            ...
            if (negative)
            {
                switch (width)
                {
                    case Width.Byte: regs.AddTo8BitRegister(dest, (byte)converted); break;
                    case Width.Word: regs.AddTo16BitRegister(dest, (ushort)converted); break;
                    case Width.TriByte: regs.AddTo24BitRegister(dest, converted); break;
                    case Width.DWord: regs.AddTo32BitRegister(dest, converted); break;
                }
            }
            else
            {
                SubtractFromRegister(cpu, width, dest, converted);
            }
        }
