...
{
    /// <summary>
    /// SR execution unit updated to use the shared Instructions sub-opcodes
    /// (the same addressing matrix used by LD / ADD / SUB / DIV / MUL / SL ...).
    ///
    /// Implements all 244 sub-op variations (0..243) via a 256-entry dispatch table.
    /// Any unassigned entries are no-ops (safety).
    ///
    /// Semantics implemented here (mirrors ExSL but shifting RIGHT):
    /// - Register destinations: shift the destination register right by a shift-count
    ///   resolved from immediate / register / memory / float-reg (per sub-op).
    /// - Memory destinations: shift the value stored at the resolved address right by
    ///   a shift-count resolved from register / memory / float-reg (per sub-op).
    /// - Block destinations:
    ///     * Immediate count: shift the target block (big-endian unsigned integer of length "count" bytes)
    ///       right by an immediate 32-bit bit-count, with optional repeat.
    ///     * Memory-sourced count: read ONE BYTE at valueAddress as the bit-count, multiply by repeat (rrr),
    ///       and shift the target block right by that total bit-count.
    /// - Float registers (_fr_*): treated as bitwise shift of the IEEE754 payload (uint bits),
    ///   returning a float from shifted bits. (Logical shift right.)
    /// </summary>
    public static class ExSR
    {
        ...
        private static void RegisterRegisterDestinations(Action<Computer>[] table)
        {
            ...
            table[Instructions._r_n] = cpu =>
            {
                ...
                ShiftRegisterRight(cpu, Width.Byte, dest, count);
            };
            ...
            table[Instructions._r_r] = cpu =>
            {
                ...
                ShiftRegisterRight(cpu, Width.Byte, dest, regs.Get8BitRegister(src));
            };
            ...
            table[Instructions._rr_nn] = cpu =>
            {
                ...
                ShiftRegisterRight(cpu, Width.Word, dest, (byte)imm);
            };
            ...
            table[Instructions._rr_r] = cpu =>
            {
                ...
                byte dest = ReadRegIndex(mem);
                byte src = ReadRegIndex(mem);
                ShiftRegisterRight(cpu, Width.Word, dest, regs.Get8BitRegister(src));
            };
            ...
            table[Instructions._rr_rr] = cpu =>
            {
                ...
                byte dest = ReadRegIndex(mem);
                byte src = ReadRegIndex(mem);
                ShiftRegisterRight(cpu, Width.Word, dest, (byte)regs.Get16BitRegister(src));
            };
            ...
            table[Instructions._rr_fr] = cpu =>
            {
                ...
                byte fIdx = ReadFloatRegIndex(mem);
                byte count = FloatToCountByte(fregs.GetRegister(fIdx));
                ShiftRegisterRight(cpu, Width.Word, dest, count);
            };
            ...
            table[Instructions._rrr_nnn] = cpu =>
            {
                ...
                ShiftRegisterRight(cpu, Width.TriByte, dest, (byte)imm24);
            };
            ...
            table[Instructions._rrr_r] = cpu =>
            {
                ...
                byte dest = ReadRegIndex(mem);
                byte src = ReadRegIndex(mem);
                ShiftRegisterRight(cpu, Width.TriByte, dest, regs.Get8BitRegister(src));
            };
            ...
            table[Instructions._rrr_rr] = cpu =>
            {
                ...
                byte dest = ReadRegIndex(mem);
                byte src = ReadRegIndex(mem);
                ShiftRegisterRight(cpu, Width.TriByte, dest, (byte)regs.Get16BitRegister(src));
            };
            ...
            table[Instructions._rrr_rrr] = cpu =>
            {
                ...
                byte dest = ReadRegIndex(mem);
                byte src = ReadRegIndex(mem);
                ShiftRegisterRight(cpu, Width.TriByte, dest, (byte)regs.Get24BitRegister(src));
            };
            ...
            table[Instructions._rrr_fr] = cpu =>
            {
                ...
                byte fIdx = ReadFloatRegIndex(mem);
                byte count = FloatToCountByte(fregs.GetRegister(fIdx));
                ShiftRegisterRight(cpu, Width.TriByte, dest, count);
            };
            ...
            table[Instructions._rrrr_nnnn] = cpu =>
            {
                ...
                ShiftRegisterRight(cpu, Width.DWord, dest, (byte)imm32);
            };
            ...
            table[Instructions._rrrr_r] = cpu =>
            {
                ...
                byte dest = ReadRegIndex(mem);
                byte src = ReadRegIndex(mem);
                ShiftRegisterRight(cpu, Width.DWord, dest, regs.Get8BitRegister(src));
            };
            ...
            table[Instructions._rrrr_rr] = cpu =>
            {
                ...
                byte dest = ReadRegIndex(mem);
                byte src = ReadRegIndex(mem);
                ShiftRegisterRight(cpu, Width.DWord, dest, (byte)regs.Get16BitRegister(src));
            };
            ...
            table[Instructions._rrrr_rrr] = cpu =>
            {
                ...
                byte dest = ReadRegIndex(mem);
                byte src = ReadRegIndex(mem);
                ShiftRegisterRight(cpu, Width.DWord, dest, (byte)regs.Get24BitRegister(src));
            };
            ...
            table[Instructions._rrrr_rrrr] = cpu =>
            {
                ...
                byte dest = ReadRegIndex(mem);
                byte src = ReadRegIndex(mem);
                ShiftRegisterRight(cpu, Width.DWord, dest, (byte)regs.Get32BitRegister(src));
            };
            ...
            table[Instructions._rrrr_fr] = cpu =>
            {
                ...
                byte fIdx = ReadFloatRegIndex(mem);
                byte count = FloatToCountByte(fregs.GetRegister(fIdx));
                ShiftRegisterRight(cpu, Width.DWord, dest, count);
            };
        }
        ...
        private static void ShiftRegFromMem8(Computer cpu, Width destWidth, AddressResolver resolver)
        {
            ...
            ShiftRegisterRight(cpu, destWidth, dest, count);
        }
        ...
        private static void RegisterMemoryGroups(Action<Computer>[] table)
        {
            ...
            foreach (var group in groups)
            {
                ...
                // Memory-sourced block shifts with repeat in rrr (same shape as ExSL)
        ...
        {
            // SR fr, fr  (bitwise on IEEE754 bits)
            ...
            // SR fr, nnnn  (count from 32-bit immediate)
            table[Instructions._fr_nnnn] = cpu =>
            {
                ...
                uint bits = FloatPointUtils.FloatToUint(fregs.GetRegister(dest));
                bits = ShiftRightU32(bits, (byte)(shiftCount & 0xFF));
                fregs.SetRegister(dest, FloatPointUtils.UintToFloat(bits));
            };

            // SR fr, r / rr / rrr / rrrr  (count from integer regs; low byte used)
            table[Instructions._fr_r] = cpu =>
            {
                ...
                uint bits = FloatPointUtils.FloatToUint(fregs.GetRegister(dest));
                bits = ShiftRightU32(bits, regs.Get8BitRegister(src));
                fregs.SetRegister(dest, FloatPointUtils.UintToFloat(bits));
            };
            ...
            table[Instructions._fr_rr] = cpu =>
            {
                ...
                uint bits = FloatPointUtils.FloatToUint(fregs.GetRegister(dest));
                bits = ShiftRightU32(bits, (byte)regs.Get16BitRegister(src));
                fregs.SetRegister(dest, FloatPointUtils.UintToFloat(bits));
            };
            ...
            table[Instructions._fr_rrr] = cpu =>
            {
                ...
                uint bits = FloatPointUtils.FloatToUint(fregs.GetRegister(dest));
                bits = ShiftRightU32(bits, (byte)regs.Get24BitRegister(src));
                fregs.SetRegister(dest, FloatPointUtils.UintToFloat(bits));
            };
            ...
            table[Instructions._fr_rrrr] = cpu =>
            {
                ...
                uint bits = FloatPointUtils.FloatToUint(fregs.GetRegister(dest));
                bits = ShiftRightU32(bits, (byte)regs.Get32BitRegister(src));
                fregs.SetRegister(dest, FloatPointUtils.UintToFloat(bits));
            };

            // SR fr, (mem8) via shared address resolvers (count read as 8-bit from memory)
        ...
        private static void ShiftRightBigEndianInPlace(MemoryController mem, uint baseAddr, byte count, int shift)
        {
            // Shift a big-endian byte array right by "shift" bits in-place.
            ...
            // First apply whole-byte shift (toward LSB end)
            byte[] tmp = new byte[count];
            for (int i = 0; i < count; i++)
            {
                int src = i - byteShift;
                tmp[i] = (src >= 0) ? buf[src] : (byte)0;
            }
            ...
            if (bitShift != 0)
            {
                ...
                {
                    byte b = tmp[i];
                    byte newCarry = (byte)(b << (8 - bitShift));          // bits that will flow into the next byte
                    tmp[i] = (byte)((b >> bitShift) | carry);            // logical shift right + incoming carry
                    carry = newCarry;
                }
            }
        ...
        // -------------------------
        private static void ShiftRegisterRight(Computer cpu, Width width, byte regIndex, byte rawCount)
        {
            var regs = cpu.CPU.REGS;
            switch (width)
            {
                case Width.Byte: regs.Shift8BitRegisterRight(regIndex, rawCount); break;
                case Width.Word: regs.Shift16BitRegisterRight(regIndex, rawCount); break;
                case Width.TriByte: regs.Shift24BitRegisterRight(regIndex, rawCount); break;
                case Width.DWord: regs.Shift32BitRegisterRight(regIndex, rawCount); break;
            }
        }
        ...
        private static uint ShiftRightU32(uint v, byte rawCount)
        {
