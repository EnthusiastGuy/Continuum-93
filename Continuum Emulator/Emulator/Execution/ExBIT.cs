...
{
    /// <summary>
    /// BIT execution unit updated to use the shared Instructions sub-opcodes
    /// (the same addressing matrix used by LD / ADD / SUB / DIV / MUL / SL / SR ...),
    /// mirroring the 244-variant dispatch approach used by ExSET.
    ///
    /// Implements all 244 sub-op variations (0..243) via a 256-entry dispatch table.
    /// Any unassigned entries are no-ops (safety).
    ///
    /// Semantics implemented here (bit-test):
    /// - Register destinations: test (flags only) the bit indexed by a bit-position resolved from
    ///   immediate / register / memory / float-reg (per sub-op), in the destination register.
    /// - Memory destinations: test (flags only) the bit indexed by the resolved bit-position in the
    ///   value stored at the resolved address (width per sub-op). Memory is NOT modified.
    /// - Block destinations:
    ///     * Immediate bit index: test one bit inside a big-endian unsigned integer block of length "count" bytes,
    ///       where bitIndex is a 32-bit immediate. With "repeat" (rrr) we test a run of bits starting at bitIndex
    ///       (flags updated per-bit; final flags reflect the last tested bit).
    ///     * Memory-sourced bit index: read ONE BYTE at valueAddress as the bitIndex, multiply by repeat (rrr),
    ///       and test the resulting bit inside the target block (run semantics as above).
    /// - Float registers (_fr_*): treated as bitwise operation on the IEEE754 payload (uint bits),
    ///   testing a bit and updating flags (float value NOT modified).
    /// </summary>
    public static class ExBIT
    {
        ...
        private static void RegisterRegisterDestinations(Action<Computer>[] table)
        {
            ...
            table[Instructions._r_n] = cpu =>
            {
                ...
                TestRegisterBit(cpu, Width.Byte, dest, bitIndex);
            };
            ...
            table[Instructions._r_r] = cpu =>
            {
                ...
                TestRegisterBit(cpu, Width.Byte, dest, regs.Get8BitRegister(src));
            };

            table[Instructions._r_InnnI] = cpu => TestRegBitFromMem8(cpu, Width.Byte, AddressAbs);
            table[Instructions._r_Innn_nnnI] = cpu => TestRegBitFromMem8(cpu, Width.Byte, AddressAbsOffsetImm);
            table[Instructions._r_Innn_rI] = cpu => TestRegBitFromMem8(cpu, Width.Byte, AddressAbsOffsetReg8);
            table[Instructions._r_Innn_rrI] = cpu => TestRegBitFromMem8(cpu, Width.Byte, AddressAbsOffsetReg16);
            table[Instructions._r_Innn_rrrI] = cpu => TestRegBitFromMem8(cpu, Width.Byte, AddressAbsOffsetReg24);

            table[Instructions._r_IrrrI] = cpu => TestRegBitFromMem8(cpu, Width.Byte, AddressPtr);
            table[Instructions._r_Irrr_nnnI] = cpu => TestRegBitFromMem8(cpu, Width.Byte, AddressPtrOffsetImm);
            table[Instructions._r_Irrr_rI] = cpu => TestRegBitFromMem8(cpu, Width.Byte, AddressPtrOffsetReg8);
            table[Instructions._r_Irrr_rrI] = cpu => TestRegBitFromMem8(cpu, Width.Byte, AddressPtrOffsetReg16);
            table[Instructions._r_Irrr_rrrI] = cpu => TestRegBitFromMem8(cpu, Width.Byte, AddressPtrOffsetReg24);
            ...
            table[Instructions._rr_nn] = cpu =>
            {
                ...
                TestRegisterBit(cpu, Width.Word, dest, (byte)imm);
            };
            ...
            table[Instructions._rr_r] = cpu =>
            {
                ...
                byte dest = ReadRegIndex(mem);
                byte src = ReadRegIndex(mem);
                TestRegisterBit(cpu, Width.Word, dest, regs.Get8BitRegister(src));
            };
            ...
            table[Instructions._rr_rr] = cpu =>
            {
                ...
                byte dest = ReadRegIndex(mem);
                byte src = ReadRegIndex(mem);
                TestRegisterBit(cpu, Width.Word, dest, (byte)regs.Get16BitRegister(src));
            };

            table[Instructions._rr_InnnI] = cpu => TestRegBitFromMem8(cpu, Width.Word, AddressAbs);
            table[Instructions._rr_Innn_nnnI] = cpu => TestRegBitFromMem8(cpu, Width.Word, AddressAbsOffsetImm);
            table[Instructions._rr_Innn_rI] = cpu => TestRegBitFromMem8(cpu, Width.Word, AddressAbsOffsetReg8);
            table[Instructions._rr_Innn_rrI] = cpu => TestRegBitFromMem8(cpu, Width.Word, AddressAbsOffsetReg16);
            table[Instructions._rr_Innn_rrrI] = cpu => TestRegBitFromMem8(cpu, Width.Word, AddressAbsOffsetReg24);

            table[Instructions._rr_IrrrI] = cpu => TestRegBitFromMem8(cpu, Width.Word, AddressPtr);
            table[Instructions._rr_Irrr_nnnI] = cpu => TestRegBitFromMem8(cpu, Width.Word, AddressPtrOffsetImm);
            table[Instructions._rr_Irrr_rI] = cpu => TestRegBitFromMem8(cpu, Width.Word, AddressPtrOffsetReg8);
            table[Instructions._rr_Irrr_rrI] = cpu => TestRegBitFromMem8(cpu, Width.Word, AddressPtrOffsetReg16);
            table[Instructions._rr_Irrr_rrrI] = cpu => TestRegBitFromMem8(cpu, Width.Word, AddressPtrOffsetReg24);
            ...
            table[Instructions._rr_fr] = cpu =>
            {
                ...
                byte fIdx = ReadFloatRegIndex(mem);
                byte bitIndex = FloatToIndexByte(fregs.GetRegister(fIdx));
                TestRegisterBit(cpu, Width.Word, dest, bitIndex);
            };
            ...
            table[Instructions._rrr_nnn] = cpu =>
            {
                ...
                TestRegisterBit(cpu, Width.TriByte, dest, (byte)imm24);
            };
            ...
            table[Instructions._rrr_r] = cpu =>
            {
                ...
                byte dest = ReadRegIndex(mem);
                byte src = ReadRegIndex(mem);
                TestRegisterBit(cpu, Width.TriByte, dest, regs.Get8BitRegister(src));
            };
            ...
            table[Instructions._rrr_rr] = cpu =>
            {
                ...
                byte dest = ReadRegIndex(mem);
                byte src = ReadRegIndex(mem);
                TestRegisterBit(cpu, Width.TriByte, dest, (byte)regs.Get16BitRegister(src));
            };
            ...
            table[Instructions._rrr_rrr] = cpu =>
            {
                ...
                byte dest = ReadRegIndex(mem);
                byte src = ReadRegIndex(mem);
                TestRegisterBit(cpu, Width.TriByte, dest, (byte)regs.Get24BitRegister(src));
            };

            table[Instructions._rrr_InnnI] = cpu => TestRegBitFromMem8(cpu, Width.TriByte, AddressAbs);
            table[Instructions._rrr_Innn_nnnI] = cpu => TestRegBitFromMem8(cpu, Width.TriByte, AddressAbsOffsetImm);
            table[Instructions._rrr_Innn_rI] = cpu => TestRegBitFromMem8(cpu, Width.TriByte, AddressAbsOffsetReg8);
            table[Instructions._rrr_Innn_rrI] = cpu => TestRegBitFromMem8(cpu, Width.TriByte, AddressAbsOffsetReg16);
            table[Instructions._rrr_Innn_rrrI] = cpu => TestRegBitFromMem8(cpu, Width.TriByte, AddressAbsOffsetReg24);

            table[Instructions._rrr_IrrrI] = cpu => TestRegBitFromMem8(cpu, Width.TriByte, AddressPtr);
            table[Instructions._rrr_Irrr_nnnI] = cpu => TestRegBitFromMem8(cpu, Width.TriByte, AddressPtrOffsetImm);
            table[Instructions._rrr_Irrr_rI] = cpu => TestRegBitFromMem8(cpu, Width.TriByte, AddressPtrOffsetReg8);
            table[Instructions._rrr_Irrr_rrI] = cpu => TestRegBitFromMem8(cpu, Width.TriByte, AddressPtrOffsetReg16);
            table[Instructions._rrr_Irrr_rrrI] = cpu => TestRegBitFromMem8(cpu, Width.TriByte, AddressPtrOffsetReg24);
            ...
            table[Instructions._rrr_fr] = cpu =>
            {
                ...
                byte fIdx = ReadFloatRegIndex(mem);
                byte bitIndex = FloatToIndexByte(fregs.GetRegister(fIdx));
                TestRegisterBit(cpu, Width.TriByte, dest, bitIndex);
            };
            ...
            table[Instructions._rrrr_nnnn] = cpu =>
            {
                ...
                TestRegisterBit(cpu, Width.DWord, dest, (byte)imm32);
            };
            ...
            table[Instructions._rrrr_r] = cpu =>
            {
                ...
                byte dest = ReadRegIndex(mem);
                byte src = ReadRegIndex(mem);
                TestRegisterBit(cpu, Width.DWord, dest, regs.Get8BitRegister(src));
            };
            ...
            table[Instructions._rrrr_rr] = cpu =>
            {
                ...
                byte dest = ReadRegIndex(mem);
                byte src = ReadRegIndex(mem);
                TestRegisterBit(cpu, Width.DWord, dest, (byte)regs.Get16BitRegister(src));
            };
            ...
            table[Instructions._rrrr_rrr] = cpu =>
            {
                ...
                byte dest = ReadRegIndex(mem);
                byte src = ReadRegIndex(mem);
                TestRegisterBit(cpu, Width.DWord, dest, (byte)regs.Get24BitRegister(src));
            };
            ...
            table[Instructions._rrrr_rrrr] = cpu =>
            {
                ...
                byte dest = ReadRegIndex(mem);
                byte src = ReadRegIndex(mem);
                TestRegisterBit(cpu, Width.DWord, dest, (byte)regs.Get32BitRegister(src));
            };

            table[Instructions._rrrr_InnnI] = cpu => TestRegBitFromMem8(cpu, Width.DWord, AddressAbs);
            table[Instructions._rrrr_Innn_nnnI] = cpu => TestRegBitFromMem8(cpu, Width.DWord, AddressAbsOffsetImm);
            table[Instructions._rrrr_Innn_rI] = cpu => TestRegBitFromMem8(cpu, Width.DWord, AddressAbsOffsetReg8);
            table[Instructions._rrrr_Innn_rrI] = cpu => TestRegBitFromMem8(cpu, Width.DWord, AddressAbsOffsetReg16);
            table[Instructions._rrrr_Innn_rrrI] = cpu => TestRegBitFromMem8(cpu, Width.DWord, AddressAbsOffsetReg24);

            table[Instructions._rrrr_IrrrI] = cpu => TestRegBitFromMem8(cpu, Width.DWord, AddressPtr);
            table[Instructions._rrrr_Irrr_nnnI] = cpu => TestRegBitFromMem8(cpu, Width.DWord, AddressPtrOffsetImm);
            table[Instructions._rrrr_Irrr_rI] = cpu => TestRegBitFromMem8(cpu, Width.DWord, AddressPtrOffsetReg8);
            table[Instructions._rrrr_Irrr_rrI] = cpu => TestRegBitFromMem8(cpu, Width.DWord, AddressPtrOffsetReg16);
            table[Instructions._rrrr_Irrr_rrrI] = cpu => TestRegBitFromMem8(cpu, Width.DWord, AddressPtrOffsetReg24);
            ...
            table[Instructions._rrrr_fr] = cpu =>
            {
                ...
                byte fIdx = ReadFloatRegIndex(mem);
                byte bitIndex = FloatToIndexByte(fregs.GetRegister(fIdx));
                TestRegisterBit(cpu, Width.DWord, dest, bitIndex);
            };
        }

        private static void TestRegBitFromMem8(Computer cpu, Width destWidth, AddressResolver resolver)
        {
            var mem = cpu.MEMC;
            var regs = cpu.CPU.REGS;
            byte dest = ReadRegIndex(mem);
            ...
            TestRegisterBit(cpu, destWidth, dest, bitIndex);
        }
        ...
        private static void RegisterMemoryGroups(Action<Computer>[] table)
        {
            ...
            foreach (var group in groups)
            {
                ...
                // Register->memory bit-test (bit index from register; low byte used)
                ...
                // Memory-sourced block bit index with repeat in rrr (same shape as ExSR pattern)
                ...
                // Float->memory bit-test: treat memory float as bits, test bit by float-reg-derived index
                table[group.FloatOpcode] = cpu =>
                {
                    ...
                    TestValue(cpu, Width.DWord, bits, bitIndex);
                };
            }
        }

        // -------------------------
        // Float register destinations (_fr_*): test on IEEE754 bits (float NOT modified)
        // -------------------------
        private static void RegisterFloatRegisterHandlers(Action<Computer>[] table)
        {
            // BIT fr, fr  (bit index from src float; tests dest float bits)
            ...
            // BIT fr, nnnn  (bit index from 32-bit immediate; low byte used)
            table[Instructions._fr_nnnn] = cpu =>
            {
                ...
                uint bits = FloatPointUtils.FloatToUint(fregs.GetRegister(dest));
                TestValue(cpu, Width.DWord, bits, (byte)(bitIndex & 0xFF));
            };

            // BIT fr, r / rr / rrr / rrrr  (bit index from integer regs; low byte used)
            table[Instructions._fr_r] = cpu =>
            {
                ...
                uint bits = FloatPointUtils.FloatToUint(fregs.GetRegister(dest));
                TestValue(cpu, Width.DWord, bits, regs.Get8BitRegister(src));
            };
            ...
            table[Instructions._fr_rr] = cpu =>
            {
                ...
                uint bits = FloatPointUtils.FloatToUint(fregs.GetRegister(dest));
                TestValue(cpu, Width.DWord, bits, (byte)regs.Get16BitRegister(src));
            };
            ...
            table[Instructions._fr_rrr] = cpu =>
            {
                ...
                uint bits = FloatPointUtils.FloatToUint(fregs.GetRegister(dest));
                TestValue(cpu, Width.DWord, bits, (byte)regs.Get24BitRegister(src));
            };
            ...
            table[Instructions._fr_rrrr] = cpu =>
            {
                ...
                uint bits = FloatPointUtils.FloatToUint(fregs.GetRegister(dest));
                TestValue(cpu, Width.DWord, bits, (byte)regs.Get32BitRegister(src));
            };

            // BIT fr, (mem8) via shared address resolvers (bit index read as 8-bit from memory)
        ...
        private static Action<Computer> MemHandler(Width width, Func<MemoryController, Registers, (uint address, uint bitIndex)> resolver)
        {
            return cpu =>
            {
                ...
                TestMemoryBit(cpu, width, address, (byte)(bitIndex & 0xFF));
            };
        }
        ...
        private static Action<Computer> BlockHandler(Func<MemoryController, Registers, (uint address, uint valueOrAddress, byte count, uint repeat, bool sourceIsAddress)> resolver)
        {
            return cpu =>
            {
                ...
                TestBlockBit(cpu, address, valueOrAddress, count, repeat, sourceIsAddress);
            };
        }

        private static void TestMemoryBit(Computer cpu, Width width, uint address, byte rawBitIndex)
        {
            var mem = cpu.MEMC;
                ...
                case Width.Byte:
                    {
                        ...
                        TestValue(cpu, width, v, rawBitIndex);
                        break;
                    }
                case Width.Word:
                    {
        ...
        private static void TestBlockBit(Computer cpu, uint address, uint valueOrAddress, byte count, uint repeat, bool sourceIsAddress)
        {
            var mem = cpu.MEMC;
            ...
            {
                // REQUIRED SEMANTICS (mirrors ExSR/ExSET pattern): bit index is a SINGLE BYTE at valueOrAddress
                bitIndex = mem.Get8bitFromRAM(valueOrAddress);
            }

            // With repeat we test a run of bits starting at bitIndex.
            // Flags will reflect the LAST tested bit (consistent with "repeat" loops elsewhere).
            for (uint i = 0; i < repeat; i++)
            {
                uint idx = (bitIndex + i) % bitWidth;
                bool set = GetBitBigEndian(mem, address, count, (int)idx);
                // Treat as 1-bit value test: Z flag typically set when bit is 0.
                // Centralize the exact flag semantics inside Registers via Test8BitValue if possible.
                cpu.CPU.REGS.Test8BitValue((byte)(set ? 1 : 0), 0);
            }
        }

        private static bool GetBitBigEndian(MemoryController mem, uint baseAddr, byte countBytes, int bitIndex)
        {
        ...
        // -------------------------
        // Core register bit-test helper
        // -------------------------
        private static void TestRegisterBit(Computer cpu, Width width, byte regIndex, byte rawBitIndex)
        {
            var regs = cpu.CPU.REGS;

            switch (width)
            {
                case Width.Byte: regs.Test8BitBit(regIndex, rawBitIndex); break;
                case Width.Word: regs.Test16BitBit(regIndex, rawBitIndex); break;
                case Width.TriByte: regs.Test24BitBit(regIndex, rawBitIndex); break;
                case Width.DWord: regs.Test32BitBit(regIndex, rawBitIndex); break;
            }
        }

        // -------------------------
        // Value-based bit-test helper (used for memory/float/block paths)
        // -------------------------
        private static void TestValue(Computer cpu, Width width, uint value, byte rawBitIndex)
        {
            var regs = cpu.CPU.REGS;
            ...
            {
                case Width.Byte:
                    regs.Test8BitValue((byte)(value & 0xFF), rawBitIndex);
                    break;
                case Width.Word:
                    regs.Test16BitValue((ushort)(value & 0xFFFF), rawBitIndex);
                    break;
                case Width.TriByte:
                    regs.Test24BitValue(value & 0xFFFFFFu, rawBitIndex);
                    break;
                case Width.DWord:
                    regs.Test32BitValue(value, rawBitIndex);
                    break;
            }
        }
